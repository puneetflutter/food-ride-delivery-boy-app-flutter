      ...
      if (variationTypes.length ==
        ...
        orderDetails.foodDetails?.choiceOptions?.forEach((choice) {
          _variationText = '$_variationText'
              '${(_index == 0) ? '' : ',  '}${choice.title} - ${variationTypes[_index]}';
          _index = _index + 1;
        });
      } else {
            ...
            Row(children: [
              Text(
                PriceConverter.convertPrice(
                    double.parse('${orderDetails.price}') - double.parse('${orderDetails.discountOnFood}')),
                style: robotoMedium,
              ),
              SizedBox(width: 5),
              orderDetails.discountOnFood! > 0
                  ? Expanded(
                      child: Text(
                      PriceConverter.convertPrice(orderDetails.price ??0),
                      style: robotoMedium.copyWith(
