...
class SplashController extends GetxController implements GetxService {
  ...
  SplashController({this.splashRepo}) : _configModel = ConfigModel();
  ...
  bool isRestaurantClosed() {
    DateTime _open = DateFormat('hh:mm').parse('00:00');
    DateTime _close = DateFormat('hh:mm').parse('00:00');
  ...
  versionCheckFun(mapdata) async {
    Response response = await ApiClient(
            appBaseUrl: AppConstants.BASE_URL, sharedPreferences: Get.find())
        .postData("/api/v1/version_check", mapdata);
