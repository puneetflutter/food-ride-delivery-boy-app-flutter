...
class PriceConverter {
  static String convertPrice(double price,
      {double discount = 0, String discountType = '', int asFixed = 0}) {
    ...
    return '${Get.find<SplashController>().configModel.currencySymbol} '
        '${(price).toStringAsFixed(asFixed ?? Get.find<SplashController>().configModel.digitAfterDecimalPoint??0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
  }
